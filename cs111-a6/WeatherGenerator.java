...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for(int i = 2; i < drywet[0].length; i++){
            drywetProbability[i-2] = drywet[drywetIndex][i];
        }

        for(int i = 2; i < wetwet[0].length; i++){
            wetwetProbability[i-2] = wetwet[wetwetIndex][i];
        }
    }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        if(mode == WET){
            ...
            for(int i = 1; i < forecast.length; i++){
                if(forecast[i] == WET && count == 0){
                    count++;
                }
                else if(forecast[i] == WET && forecast[i-1] == WET){
                    count++;
                }
                else{
                    if(longest < count){
            ...
            for(int i = 1; i < forecast.length; i++){
                if(forecast[i] == DRY && count == 0){
                    count++;
                }
                else if(forecast[i] == DRY && forecast[i - 1] == DRY){
                    count++;
                }
                else{
                    if(longest < count){
