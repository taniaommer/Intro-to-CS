public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        //name
        s = description.substring(description.indexOf("name:") + 5);
        String name = s.substring(0, s.indexOf(";"));


        //fuel
        s = description.substring(description.indexOf("type:") + 5);
        String type = s.substring(0, s.indexOf(";"));

        s = description.substring(description.indexOf("mile/unit:") + 10);
        String usage = s.substring(0, s.indexOf(";"));

        String charger = "";
        if(description.indexOf("charger:") != -1){
            s = description.substring(description.indexOf("charger:") + 8);
            charger = s.substring(0, s.indexOf(";"));
        }


        //lease
        s = description.substring(description.indexOf("due:") + 4);
        String dueAtSigning = s.substring(0, s.indexOf(";"));

        s = description.substring(description.indexOf("length:") + 7);
        String length = s.substring(0, s.indexOf(";"));

        s = description.substring(description.indexOf("monthly:") + 8);
        String monthlyCost = s.substring(0, s.indexOf(";"));

        s = description.substring(description.indexOf("allowance:") + 10);
        String allowance = s.substring(0, s.indexOf(";"));

	    Vehicle newVehicle;
        Lease newLease = new Lease(Double.parseDouble(dueAtSigning), Integer.parseInt(length), Double.parseDouble(monthlyCost), Integer.parseInt(allowance));
        if (type.equals("gas")) {
            Fuel gasFuel = new Fuel(Double.parseDouble(usage));
            newVehicle = new Vehicle(name, gasFuel, newLease);
        } 
        else {
            Fuel elecFuel = new Fuel(Double.parseDouble(usage), Double.parseDouble(charger));
            newVehicle = new Vehicle(name, elecFuel, newLease);
        }
        return newVehicle;
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for(Vehicle vehicle : vehicles){
            ...
            int type = vehicle.getFuel().getType();
            ...
            if(type == Fuel.GAS){
                ...
                fuelCost = computeFuelCost(vehicle.getLease().getLeaseLength(), vehicle.getFuel().getUsage(), vehicle.getLease().getMileageAllowance(), gasPrice);
            ...
            if(type == Fuel.ELECTRIC){
                ...
                fuelCost = computeFuelCost(vehicle.getLease().getLeaseLength(), vehicle.getFuel().getUsage(), vehicle.getLease().getMileageAllowance(), electricityPrice);
                vehicle.setFuelCost(fuelCost);
                double charger = vehicle.getFuel().getCharger();
                vehicle.setTotalCost(fuelCost + leaseCost + charger);
            }
